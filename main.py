...
def get_sucursal_address_1(sucursal_id:int):
    linea1 = 'Sucursal : '+str.upper(sucursales.iloc[sucursal_id,1])
    linea2 = 'Dirección: '+sucursales.iloc[sucursal_id,2]
    valret = f"{linea1}    {linea2}"
    return valret

@app.get('/sucursales_name')
def get_sucursal_address_2(sucursal_name: str= Query(None, description=f"Ingrese desde las 3 primeras letras de los valores posibles de esta lista: {lst_suc_names}")):
